...
class Bot:
	...
	def answer(self, update):
		...
		if query.startswith('/start') or query.startswith('/help'):
			...
			if query.startswith('/mp3') or query.startswith('/mp4'):
				thread = threading.Thread(target=self.idle_download, args=(userId, query))
				thread.start()
			else:
				self.bot.sendMessage(userId, 'what?')
		elif current_state == WAITING_LINK:
			thread = threading.Thread(target=self.waiting_download, args=(userId, self.states[userId]['query'], query))
			thread.start()
	...
	def waiting_download(self, userId, kind, link):
		try:
			...
			self.bot.sendMessage(userId, 'unfortunately there was a problem with your request')
		self.states[userId] = { 'state': IDLE_STATE }
